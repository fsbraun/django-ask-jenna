...
class Prompts:
    ...
    def get(self, view, opts: models.options.Options, instance: Optional[models.Model] = None):
        ...
        for field, field_prompt in prompt.items():
            if callable(field_prompt.get("dynamic_content")):
                field_prompt["dynamic_content"] = field_prompt["dynamic_content"](instance)
            field_prompt["prompt"] = Template(field_prompt["prompt"]).render(Context({
                "instance": instance,
                **field_prompt,
            }))
        return prompt
...
if not prompts:
    for app_config in apps.get_app_configs():
        ...
        try:
            module = import_module("%s.%s" % (app_config.name, "ask_jenna"))
